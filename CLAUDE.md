...
- **Note**: `ContentIngestionService` is not exported from `src/services/index.ts`; import directly:
  ```typescript
...
- **Tab bar order** (left to right): `✕ MARKS` | `▦ LOG` | `◿ RECENT` | `⌗ WINDOWS` | `⚙ DIAG`
...
- **▦ LOG**: X bookmark analytics dashboard (metrics, timelines, top authors, content composition)
...
- Summary stats: total bookmarks, archived count, unique authors, bookmarks/week, media/video/tagged/quote percentages
- Acquisition timeline: horizontal bar chart of bookmarks per week
- Top authors: horizontal bar chart of most-bookmarked authors
- Content composition: gradient bar showing text-only vs images vs video
- Tweet age distribution: horizontal bar chart of original tweet dates by month
- Fetches data via `X_GET_BOOKMARK_METRICS` message
- NASA instrument panel styling (monospace labels, green/amber accents)
