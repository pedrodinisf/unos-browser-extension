...
Actions:
  ...
  - write_engine_cookies:  Write an X cookie jar (Netscape format) for the media_engine server
  - clear_engine_cookies:  Delete that cookie jar
"""
import json
import logging
import os
...
# Cookie jar consumed by the media_engine server (same user, local machine).
# Written on demand, chmod 0600, deleted after the transfer completes.
ENGINE_COOKIE_FILE = SCRIPT_DIR / "engine-cookies.txt"
...
def action_write_engine_cookies(msg):
    """Write an X cookie jar (Netscape format) for the media_engine server.

    The jar path is stable so the server can read it while jobs run; the
    extension clears it when the transfer finishes. File mode is 0600;
    cookie values are never logged.
    """
    cookies = msg.get("cookies", []) or []
    try:
        fd = os.open(
            str(ENGINE_COOKIE_FILE),
        ...
        with os.fdopen(fd, "w", encoding="utf-8") as f:
            f.write(cookies_to_netscape(cookies))
        try:
            os.chmod(ENGINE_COOKIE_FILE, 0o600)
        except OSError:
            pass

        log.info(
            "write_engine_cookies: %d cookies -> %s",
            len(cookies),
            ENGINE_COOKIE_FILE,
        )
        return {
            "success": True,
            "cookiePath": str(ENGINE_COOKIE_FILE),
            "cookieCount": len(cookies),
        }
    except Exception as e:
        # Don't leave a partial jar behind for the engine to read.
        try:
            Path(ENGINE_COOKIE_FILE).unlink(missing_ok=True)
        except OSError:
            pass
        log.error("write_engine_cookies failed: %s", e)
        return {"success": False, "error": f"Could not write cookie jar: {e}"}


def action_clear_engine_cookies():
    """Delete the media_engine cookie jar (best effort)."""
    try:
        Path(ENGINE_COOKIE_FILE).unlink(missing_ok=True)
        log.info("clear_engine_cookies: removed %s", ENGINE_COOKIE_FILE)
        return {"success": True}
    except Exception as e:
        log.warning("clear_engine_cookies failed: %s", e)
        return {"success": False, "error": str(e)}
...
def main():
    ...
    elif action == "write_engine_cookies":
        result = action_write_engine_cookies(msg)
        log.info("Result: %s", result)
        send_response(result)

    elif action == "clear_engine_cookies":
        result = action_clear_engine_cookies()
        log.info("Result: %s", result)
        send_response(result)
